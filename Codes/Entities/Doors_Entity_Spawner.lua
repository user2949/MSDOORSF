-- https://raw.githubusercontent.com/RegularVynixu/Utilities/main/Doors%20Entity%20Spawner/Source.lua - original (i just changed some stuff)
-- all credits to vynixu check the original out
    ...
    entityConnections.movementTick = RS.Stepped:Connect(function()
        if entityModel.Parent and not entityModel:GetAttribute("NoAI") then
            ...
            if playerInSight then
                ...
                -- Kill player
                local entityRootModel = nil
                if entityModel.PrimaryPart then
                 entityRootModel = entityModel.PrimaryPart
                elseif entityModel:FindFirstChild("Hitbox") then
                ...
                if entityTable.Config.CanKill and not Char:GetAttribute("IsDead") and not Char:GetAttribute("Invincible") and not Char:GetAttribute("Hiding") and not Char:FindFirstChild("Crucifix") and (getPlayerRoot().Position - entityRootModel.Position).Magnitude <= entityTable.Config.KillRange then
                    task.spawn(function()
                        ...
                        if #entityTable.Config.CustomDialog > 0 then
                            debug.setupvalue(getconnections(game:GetService("ReplicatedStorage").EntityInfo.DeathHint.OnClientEvent)[1].Function, 1, entityTable.Config.CustomDialog)
                        end
